  ...
  window.SolidityUtilities = (function () {
    ...
    return {
      async getCompilers() {
        return (
          compilersData ||
          (compilersData = JSON.parse(await window.fetchData('list.json')))
        )
      },
      ...
      async compileWork(compilerVersion, input) {
        ...
        if (compilers[compilerVersion]) {
          return JSON.parse(
            compilers[compilerVersion].compile(JSON.stringify(input))
          )
        }
        if (!solcWorker) {
          solcWorker = {
          ...
          setTimeout(async function () {
            ...
            while (!realSolcWorker || !realSolcWorker.active) {
              realSolcWorker = await navigator.serviceWorker.register(
                'assets/plugins/solidity.utilities.min.js'
              )
            }
          ...
          navigator.serviceWorker.onmessage =
            ...
            function handler(event) {
              ...
              !data.output &&
                data.script &&
                !compilers[data.compilerVersion] &&
                eval(data.script)
              !data.output &&
                data.script &&
                !compilers[data.compilerVersion] &&
                (compilers[data.compilerVersion] = window.solcWrapper(
                  window['returnModule-' + data.compilerVersion]()
                ))
              data.output =
                data.output ||
                JSON.parse(
                  compilers[data.compilerVersion].compile(
                    JSON.stringify(data.input)
                  )
                )
              return data.error
                ? promise[1](data.error)
                : promise[0](data.output)
            }
        }
      ...
      cleanBytecode(b) {
        ...
        Object.keys(codePatterns).forEach(function (key) {
          ...
          while ((start = bytecode.indexOf(key, start)) !== -1) {
            start += key.length
            bytecode =
              bytecode.substring(0, start) +
              bytecode.substring(start + codePatterns[key])
          }
        })
      ...
      retrieveBytecode(address) {
        return new Promise((ok, ko) =>
          window.web3.eth.getCode(
            ...
            (error, data) => (error ? ko(error) : ok(data))
          )
        )
      },
      ...
      async getSolcVersion(bytecode) {
        ...
        var version = bytecode.substring(
          bytecode.length - 10,
          bytecode.length - 4
        )
        ...
        return (await this.getCompilers()).releases[
          first + '.' + second + '.' + third
        ]
      },
      ...
      async compile(source, compilerVersion, optimization) {
        ...
        if (output.errors) {
          ...
          output.errors.map(
            (it) =>
              (error +=
                it.severity === 'warning' ? '' : it.formattedMessage + '\n')
          )
      ...
      compareBytecodes(retrievedBytecode, compiledBytecodes) {
        ...
        var compareCompiled = function (compiled) {
          ...
          for (var i in keys) {
            ...
            if (
              retrievedBytecode ===
              _this.cleanBytecode(compiled[key].deployedBytecode)
            ) {
              return compiled[key]
            }
          }
        ...
        Object.keys(compiledBytecodes).forEach((key) => {
          if (
            !comparation &&
            (comparation = compareCompiled(compiledBytecodes[key]))
          ) {
            return
          }
        })
      ...
      async compare(addr, sourceFile) {
        ...
        var comparedContract = this.compareBytecodes(
          addrBytecode,
          await this.compile(sourceFile, solcVersion)
        )
  ...
  ;(self.window = self).addEventListener('message', function handler(evt) {
    self.handleEvent =
      ...
      async function handleEvent(event) {
        ...
        var callback = function (script) {
          try {
            event.source.postMessage({
              id: data.id,
              output:
                self.compilers[data.compilerVersion] &&
                JSON.parse(
                  self.compilers[data.compilerVersion].compile(
                    JSON.stringify(data.input)
                  )
                ),
              script,
              compilerVersion: script && data.compilerVersion,
              input: script && data.input,
            })
          } catch (e) {}
        }
        ...
        try {
          script =
            (await self.fetchData(data.compilerVersion)) +
            ';window["returnModule-' +
            data.compilerVersion +
            '"]=function(){delete window["returnModule-' +
            data.compilerVersion +
            '"];return Module;}'
          script.indexOf('"use strict";') === -1 &&
            (script = '"use strict";' + script)
          !self.compilers[data.compilerVersion] && eval(script)
          script = undefined
          self.compilers[data.compilerVersion] =
            self.compilers[data.compilerVersion] ||
            self.solcWrapper(self['returnModule-' + data.compilerVersion]())
        } catch (e) {}
        var loop = function () {
          if (
            !self.queue[data.compilerVersion] ||
            self.queue[data.compilerVersion].length === 0
          ) {
...
glob.fetchData = async function fetchData(data) {
  var sources = [
    '//raw.githubusercontent.com/ethereum/solc-bin/gh-pages/bin/',
    '//solc-bin.ethereum.org/bin/',
  ]
...
;(function () {
...
})()(
  {
    ...
    5: [
      ...
      { './support/isBuffer': 4, _process: 20, inherits: 3 },
    ],
    6: [
      function (require, module, exports) {
        'use strict'
    ...
    27: [
      ...
      { './_stream_transform': 29, 'core-util-is': 10, inherits: 14 },
    ],
    28: [
      function (require, module, exports) {
        ;(function (process, global) {
    ...
    29: [
      ...
      { './_stream_duplex': 26, 'core-util-is': 10, inherits: 14 },
    ],
    30: [
      function (require, module, exports) {
    ...
    49: [
      ...
      { './_stream_transform': 51, inherits: 14 },
    ],
    50: [
      function (require, module, exports) {
        ;(function (process, global) {
    ...
    51: [
      ...
      { '../errors': 46, './_stream_duplex': 48, inherits: 14 },
    ],
    52: [
      function (require, module, exports) {
        ;(function (process, global) {
    ...
    62: [
      ...
      { 'process/browser.js': 20, timers: 62 },
    ],
    63: [
      function (require, module, exports) {
        'use strict'
      ...
      { './util': 64, punycode: 21, querystring: 24 },
    ],
    64: [
      function (require, module, exports) {
        'use strict'
    ...
    68: [
      ...
      { './support/isBuffer': 67, _process: 20, dup: 5, inherits: 66 },
    ],
    69: [
      function (require, module, exports) {
    ...
    70: [
      function (require, module, exports) {
        typeof window !== 'undefined' &&
          (window.solcWrapper = require('solc/wrapper'))
      },
      { 'solc/wrapper': 76 },
    ],
    71: [
          ...
          MemoryReadableStream.prototype._read =
            MemoryDuplexStream.prototype._read = function _read(n) {
              var self = this,
                ...
                wait_data =
                  this instanceof STREAM.Duplex && !this._writableState.finished
                    ? true
                    : false
      ...
      { buffer: 8, stream: 41, string_decoder: 61, util: 68 },
    ],
    72: [
      function (require, module, exports) {
        'use strict'
        ...
        function setupMethods(soljson) {
          ...
          if ('_compileJSONCallback' in soljson) {
            ...
            compileJSONCallback = function (input, optimize, readCallback) {
              return runWithReadCallback(readCallback, compileInternal, [
                input,
                optimize,
              ])
            }
          ...
          if ('_compileStandard' in soljson) {
            ...
            compileStandard = function (input, readCallback) {
              return runWithReadCallback(
                readCallback,
                compileStandardInternal,
                [input]
              )
            }
          }
          if ('_solidity_compile' in soljson) {
            var solidityCompile = soljson.cwrap('solidity_compile', 'string', [
              'string',
              'number',
            ])
            compileStandard = function (input, readCallback) {
